...
class MCDCLIP(nn.Module):
    def __init__(self,num_class,prompt_num,prompt_dim,embedding_dim,re_com_dim,re_pri_dim,):
        super().__init__()
    ...
    def forward(self, x, mask=None):
        ...
        similarity_in = (tem * torch.bmm(view_1_cls.unsqueeze(1), text_front.transpose(1, 2)).squeeze(1))
        similarity_la = (tem * torch.bmm(view_2_cls.unsqueeze(1), text_later.transpose(1, 2)).squeeze(1))
        kekka=(similarity_in+similarity_la)/2
        return kekka
